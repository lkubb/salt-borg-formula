...
We will see a quick setup to configure the ``borg`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``borg`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``borg`` formulas under ``/srv/salt/borg/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/borg/parameters/dns:domain/
    ln -s dns:domain /srv/salt/borg/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/borg/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/borg/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``borg`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``borg`` formulas under ``/srv/salt/borg/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/borg/parameters/roles
...
We create a configuration for the role ``borg/server`` in ``/srv/salt/borg/parameters/roles/borg/server.yaml``:
...
We create another configuration for the role ``borg/client`` in ``/srv/salt/borg/parameters/roles/borg/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles borg/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles borg/client
...
.. code-block:: console
    ...
    mv /srv/salt/borg/parameters/dns:domain/  '/srv/salt/borg/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-borg-as-json:
      test.nop:
        - name: {{ borg | json }}
...
Here is an example based on `template-formula/borg/config/file.sls`_:
    ...
    borg-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='borg-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ borg.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``borg`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ borg['winner'] }}
...
.. _template-formula/borg/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/borg/config/file.sls
